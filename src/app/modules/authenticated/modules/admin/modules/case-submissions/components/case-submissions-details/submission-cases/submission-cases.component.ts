...
@Component({
  selector: 'admin-submission-cases',
  templateUrl: './submission-cases.component.html',
  styleUrls: ['./submission-cases.component.scss'],
})
export class AdminSubmissionCasesComponent implements OnInit, OnDestroy {
  ...
  downloadMedia(file_id, file_name, case_uuid) {
    this.apiService.download('admin/submissions/' + this.submission.uuid + '/cases/' + case_uuid + '/documents/' + file_id).toPromise()
      .then(blob => {
  ...
  downloadFile(case_uuid, file_type, file_name) {
    this.apiService.download('admin/portal/download/' + case_uuid + '/' + file_type).toPromise()
      .then(blob => {
        saveAs(blob, file_name);
      });
  }
  ...
  removeCase(case_uuid) {
    ...
    this.deleteSub = this.apiService.delete('admin/submissions/' + this.submission.uuid + '/cases/' + case_uuid).subscribe((res) => {
  ...
  onDropzoneSelectCaseDocuments(event, case_uuid) {
    ...
    this.uploadSub = this.apiService.upload('admin/submissions/' + this.submission.uuid + '/cases/' + case_uuid + '/documents', form_data).subscribe((res) => {
  ...
  removeMedia(media_id, case_uuid) {
    this.processing = true;
    this.deleteSub = this.apiService.delete('admin/submissions/' + this.submission.uuid + '/cases/' + case_uuid + '/documents/' + media_id).subscribe((res) => {
