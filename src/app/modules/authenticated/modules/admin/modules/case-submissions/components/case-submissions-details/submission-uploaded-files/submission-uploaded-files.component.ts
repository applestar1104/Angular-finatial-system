...
@Component({
  selector: 'admin-submission-uploaded-files',
  templateUrl: './submission-uploaded-files.component.html',
  styleUrls: ['./submission-uploaded-files.component.scss'],
})
export class AdminSubmissionUploadedFilesComponent implements OnInit, OnDestroy {
  ...
  downloadMedia(file_id, file_name) {
    this.apiService.download('admin/submissions/' + this.submission.uuid + '/media/' + file_id).toPromise()
      .then(blob => {
  ...
  onDropzoneSelectClientIdentity(event) {
    ...
    this.uploadSub = this.apiService.upload('admin/submissions/' + this.submission.uuid + '/media/client-identity', form_data).subscribe((res) => {
  ...
  onDropzoneSelectClientPFR(event) {
    ...
    this.uploadSub = this.apiService.upload('admin/submissions/' + this.submission.uuid + '/media/pfr', form_data).subscribe((res) => {
      // console.log("Uploaded files", res);
      if (res.error === false) {
        let data = res.data;
        this.media = data;
  ...
  removeMedia(media_id) {
    this.processing = true;
    this.deleteSub = this.apiService.delete('admin/submissions/' + this.submission.uuid + '/media/' + media_id).subscribe((res) => {
