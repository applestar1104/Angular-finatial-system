...
@Component({
  selector: 'admin-associate-submissions',
  templateUrl: './associate-submissions.component.html',
  styleUrls: ['./associate-submissions.component.scss'],
})
export class AdminAssociateSubmissionsComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    // Update page title
    this.titleService.manualTitle(this.associate.personal.full_name + ' | Associate Submissions');
  }
  ...
  getSubmissions() {
    if (this.associate) {
      ...
      this.dataRaw = [];
      // this.dataCategory = [];
      this.submissionsSub = this.adminSalesAssocService.getSubmissions(this.associate.uuid).subscribe(res => {
        let data = res.data;
  ...
  filterByProvider(dataset, provider = null) {
    if (provider) return dataset.filter(row => row.providers.includes(provider));
    return dataset;
  }
  ...
  routeStore() {
    this.sessionStorageService.setItem('previous_title', 'Associate Submissions Records');
