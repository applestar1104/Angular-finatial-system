...
@Component({
  selector: 'admin-associate-policies',
  templateUrl: './associate-policies.component.html',
  styleUrls: ['./associate-policies.component.scss'],
})
export class AdminAssociatePoliciesComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    // Update page title
    this.titleService.manualTitle(this.associate.personal.full_name + ' | Associate Policies');
  }
  ...
  getPolicies() {
    if (this.associate) {
      ...
      this.dataRaw = [];
      // this.dataCategory = [];
      this.policiesSub = this.adminSalesAssocService.getPolicies(this.associate.uuid).subscribe(res => {
        let data = res.data;
        this.policies = data;
        this.dataRaw = data;
        this.fuseResults();
