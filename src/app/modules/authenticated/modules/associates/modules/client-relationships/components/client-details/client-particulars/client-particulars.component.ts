...
@Component({
  selector: 'associates-client-particulars',
...
export class AssociatesClientParticularsComponent implements OnInit, OnDestroy {
  ...
  submitForm() {
    ...
    this.clientUpdateSub = this.assClientService.updateClient(this.client.uuid, this.getUpdatedValues()).subscribe((res) => {
      if (res.error === false) {
        this.router.navigate(['/authenticated/associates/relationships/clients/' + this.client.uuid + '/particulars']);
        ...
        console.log(res);
        this._notification.create('warning', 'Something went wrong.', { Position: 'top', Style: 'bar', Duration: 2000 });
