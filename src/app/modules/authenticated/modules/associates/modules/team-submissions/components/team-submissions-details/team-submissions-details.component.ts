...
@Component({
  selector: 'associates-team-submissions-details',
  templateUrl: './team-submissions-details.component.html',
  styleUrls: ['./team-submissions-details.component.scss']
})
export class AssociatesTeamSubmissionsDetailsComponent {
  page_permission = 'associate_teams_view';
  ...
  restoreSubmission() {
    this.restoreSub = this.apiService.post('associates/submissions/' + this.submission.uuid + '/restore').subscribe((res) => {
      // console.log("Restored submission", res);
      if (res.error === false) {
        this.updateModel(res['data']);
        this.router.navigate(['/authenticated/associates/case-submissions/' + this.submission.uuid]);
  ...
  updateSubmissionStatus(new_status) {
    this.processing = true;
    this.updateSub = this.apiService.post('associates/submissions/' + this.submission.uuid + '/status/' + new_status).subscribe((res) => {
